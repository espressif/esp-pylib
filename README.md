...
**Pinned stdout vs TTY progress.** `log.set_console_options(file=...)` pins default progress to that deliverable. In-place `\r` redraws require the pin target to be a terminal (`is_terminal`) — use `log.progress(..., file=sys.stderr)` when the bar should stay on the live TTY.
